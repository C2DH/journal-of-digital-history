const ArticleFingerprintCellGraphics = ({
  ...
  return (
    <g
      className={[isHeading ? 'is-heading' : '', isHermeneutic ? 'is-hl' : '', 'type-' + type].join(
        ' ',
      )}
    >
      <line x1={ox} y1={oy} x2={cx} y2={cy} strokeWidth={lineStrokeWidth} />
      <circle
        className="origin"
        cx={ox}
        cy={oy}
      ...
      {refsRadius ? (
        <circle
          className="refs"
          cx={cosTheta * (offsetRadius + originRadius + refsRadius + refsOffset)}
          cy={sinTheta * (offsetRadius + originRadius + refsRadius + refsOffset)}
          r={refsRadius}
          strokeWidth={circleStrokeWidth}
        />
      ) : null}
      <circle cx={cx} cy={cy} r={circleRadius} strokeWidth={circleStrokeWidth} />
    </g>
  )
}
