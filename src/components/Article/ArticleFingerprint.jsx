...
const ArticleFingerprint = ({
  ...
  const compactScale = Math.max(0.45, Math.min(1, size / 220))
  const markerScale = Math.max(0.35, Math.min(1, size / 260))
  const lineStrokeWidth = Math.max(0.35, 0.8 * compactScale)
  const circleStrokeWidth = Math.max(0.3, 0.75 * markerScale)
  const originDotRadius = Math.max(0.25, 0.8 * markerScale)
  const headingDotRadius = Math.max(1.4, 4.5 * markerScale)
  const circleRadius = Math.max(0.8, 2.2 * markerScale)
  const refsOffset = 4 * markerScale
  const radius = ((size / 2 - margin) * 2) / 3
  // value radius, this give us extra safety margin.
  const maxNumCharsRadius = radius / 2
  const minNumRefsRadius = Math.max(0.5, 1.3 * markerScale)
  const maxNumRefsRadius = Math.max(1.6, 4.2 * markerScale)
  ...
  const scaleNumRefs = scalePow()
    ...
    .range([minNumRefsRadius, maxNumRefsRadius])

  const scaleTheta = scalePow()
    .exponent(1)
    .domain([0, 360])
  ...
  if (debug) {
    console.info(
      ...
      '\n extentChars:',
      stats?.extentChars,
      '\n recalculated extentChars:',
      extent(cells, (d) => d.countChars),
      '\n range:',
      [0, maxNumCharsRadius],
    )
  }
  return (
    <svg
      className={`ArticleFingerprint position-absolute ${variant}`}
      ...
      <g transform={`translate(${size / 2}, ${size / 2})`}>
        ...
        */}
        {cells.map((cell, i) => {
          ...
          return (
            <ArticleFingerprintCellGraphics
              key={i}
              theta={theta}
              originRadius={radius}
              circleRadius={circleRadius}
              originDotRadius={originDotRadius}
              headingDotRadius={headingDotRadius}
              offsetRadius={
                // if isHermeneutic, inWard, if is heading is 0
                cell.isHermeneutic ? numCharsRadius * -1 : numCharsRadius
              }
