...
export const extractMetadataFromArticle = (article) => {
  ...
  const keys = ['title', 'abstract', 'contributor', 'collaborators']
  keys.forEach((key) => {
  ...
  result.excerpt = stripHtml(
    extractExcerpt(result.abstract.map((d) => d.content).join(''), 20, '...'),
  ).trim()
  ...
  result.plainContributor = result.contributor
    .map((d) => {
      return d.content
  ...
  if (Array.isArray(article.data.keywords)) {
    result.keywords = article.data.keywords
      .reduce((acc, d) => {
        return acc.concat(
          String(d)
            .trim()
...
export const stripHtml = (text) => {
  ...
  return text.replace(/<br>/gi, '').replace(/<[^>]+>/g, '')
}
